...
// // Resolve python/java/g++ command names for Windows vs Unix
// const CMD = {
//   node:   'node',
...
// /**
//  * Spawn a process with args as an array — no shell, no quoting issues.
//  * stdinData is written to the process stdin if provided.
//  */
...
//     // Normalize line endings to \n and trim, then write + close stdin
...
//       resolve({
//         stdout,
//         stderr: signal === 'SIGTERM' ? 'Time limit exceeded' : stderr,
//       })
//     })
...
// // ── Build the harness wrapper for problem judging ────────────────
// function buildRunnable(language, userCode, inputStr) {
//   const tokens = (inputStr || '').trim().split(/\s+/)

//   switch (language) {
//     case 'javascript': {
//       const args = JSON.stringify(tokens.map(t => isNaN(Number(t)) ? t : Number(t)))
...
//     fs.writeFileSync(file, code)
//     const compile = await run(CMD.javac, ['-encoding', 'UTF-8', file], { cwd: tmpDir })
//     if (compile.stderr) return { stdout: '', stderr: compile.stderr }
//     return run(CMD.java, ['-Dfile.encoding=UTF-8', '-cp', tmpDir, 'Main'])
//   }

//   if (language === 'cpp') {
//     const srcFile = path.join(tmpDir, `sol_${Date.now()}.cpp`)
//     const outFile = path.join(tmpDir, IS_WINDOWS ? `sol_${Date.now()}.exe` : `sol_${Date.now()}`)
...
//       fs.writeFileSync(file, code)
//       result = await run(CMD.node, [file], {}, stdin)
...
//       const file = path.join(tmpDir, 'Main.java')
//       fs.writeFileSync(file, code)
//       // Compile first — no stdin needed for compilation
//       const compile = await run(CMD.javac, ['-encoding', 'UTF-8', file], { cwd: tmpDir })
//       if (compile.stderr) return res.json({ stdout: '', stderr: compile.stderr })
//       result = await run(CMD.java, ['-Dfile.encoding=UTF-8', '-cp', tmpDir, 'Main'], {}, stdin)

//     } else if (language === 'cpp') {
//       const srcFile = path.join(tmpDir, 'solution.cpp')
//       const outFile = path.join(tmpDir, IS_WINDOWS ? 'solution.exe' : 'solution')
//       fs.writeFileSync(srcFile, code)
//       const compile = await run(CMD.gpp, ['-o', outFile, srcFile])
//       if (compile.stderr) return res.json({ stdout: '', stderr: compile.stderr })
...
//       try {
//         const { stdout, stderr } = await runOneCase(language, userCode, tc.input, tmpDir)
...
//         })
//       } catch (err) {
//         results.push({ input: '(hidden)', expected: '(hidden)', actual: '', passed: false, stderr: '', error: err.message })
//       }
//     }

//     res.json({ status: results.every(r => r.passed) ? 'Accepted' : 'Wrong Answer', results })
//   } catch (err) {
//     res.status(500).json({ error: err.message })
//   } finally {
//     fs.rmSync(tmpDir, { recursive: true, force: true })
    ...
    child.on('close', (code, signal) => {
      resolve({
        ...
        stderr: signal === 'SIGTERM'
          ? (killed ? 'Output limit exceeded' : 'Time limit exceeded')
          : stderr,
      })
    })
...
async function runOneCase(language, userCode, inputStr, tmpDir) {
  ...
  if (language === 'java') {
    ...
    return run(CMD.java, ['-Xmx128m', '-Dfile.encoding=UTF-8', '-cp', tmpDir, 'Main'])
  }
  if (language === 'cpp') {
    const srcFile = path.join(tmpDir, `sol_${Date.now()}.cpp`)
    const outFile = path.join(tmpDir, IS_WINDOWS ? `sol_${Date.now()}.exe` : `sol_${Date.now()}`)
...
const executeCode = async (req, res) => {
  ...
  try {
    ...
    if (language === 'javascript') {
      const file = path.join(tmpDir, 'solution.js')
      fs.writeFileSync(file, code)
      result = await run(CMD.node, ['--max-old-space-size=128', file], {}, stdin)
      ...
      const compile = await run(CMD.javac, ['-encoding', 'UTF-8', file], { cwd: tmpDir })
      if (compile.stderr) return res.json({ stdout: '', stderr: compile.stderr })
      result = await run(CMD.java, ['-Xmx128m', '-Dfile.encoding=UTF-8', '-cp', tmpDir, 'Main'], {}, stdin)
