...
const getProblems = async (req, res) => {
  try {
    const problems = await Problem.find().select('-hiddenTestCases').sort({ createdAt: -1 })
    res.json(problems)
  } catch (err) {
...
module.exports = { getProblems, getProblem, getProblemWithHidden, createProblem, updateProblem, deleteProblem }
