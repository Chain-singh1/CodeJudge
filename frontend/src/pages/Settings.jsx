...
const Settings = () => {
  ...
  const handleUsernameUpdate = async (e) => {
    ...
    if (username === user?.username)
      return setUsernameMsg({ type: 'error', text: 'Username is the same as current' })
    ...
    try {
      ...
      setUsernameMsg({ type: 'error', text: err.response?.data?.message || 'Failed to update' })
    } finally {
  ...
  const handlePasswordUpdate = async (e) => {
    ...
    try {
      ...
      setPasswordMsg({ type: 'error', text: err.response?.data?.message || 'Failed to update' })
    } finally {
  ...
  const Feedback = ({ msg }) => {
    if (!msg) return null
    return (
      <p className={`text-sm mt-2 ${msg.type === 'success' ? 'text-green-400' : 'text-red-400'}`}>
    ...
    <div className="min-h-screen w-full bg-[#0f1117] text-white">
      ...
      <div className="max-w-xl mx-auto px-6 py-8">
        ...
        <div className="bg-[#1a1d27] border border-gray-800 rounded-2xl p-5 mb-6">
          <h2 className="text-sm font-medium text-gray-300 mb-4">Account Info</h2>
        ...
        {/* Change username */}
        <div className="bg-[#1a1d27] border border-gray-800 rounded-2xl p-5 mb-6">
          <h2 className="text-sm font-medium text-gray-300 mb-4">Change Username</h2>
          <form onSubmit={handleUsernameUpdate} className="space-y-4">
            <div>
              <label className="block text-xs text-gray-500 mb-1.5">New username</label>
              <input
                className={inputCls}
                value={username}
