...
//   // Count down to start if upcoming, to end if active
//   const target   = isUpcoming ? contest.startTime : isActive ? contest.endTime : null
//   const timeLeft = useCountdown(target)

//   if (!target || !timeLeft) return null

//   const urgent = isActive && timeLeft.diff < 5 * 60 * 1000  // < 5 min
...
//     }`}>
//       <div className={`text-xs font-medium ${urgent ? 'text-red-400' : isActive ? 'text-green-400' : 'text-blue-400'}`}>
//         {isUpcoming ? 'Starts in' : urgent ? '⚠ Ending soon' : 'Time left'}
//       </div>
//       <div className={`font-mono font-bold text-lg tracking-widest ${
//         urgent ? 'text-red-300' : isActive ? 'text-green-300' : 'text-blue-300'
//       }`}>
...
//   useEffect(() => { fetchContest(); fetchSubmissions() }, [id])
...
//       if (!uid) return acc
//       if (!acc[uid]) acc[uid] = { user: s.user, solved: new Set(), total: 0 }
...
//   const now      = new Date()
//   const isActive = contest && new Date(contest.startTime) <= now && new Date(contest.endTime) > now
...
//     <div className="min-h-screen w-full bg-[#0f1117] flex flex-col">
//       <Navbar />
//       <div className="flex-1 flex items-center justify-center text-gray-500 text-sm">Contest not found</div>
//     </div>
//   )
...
//                   isActive ? 'text-green-400 bg-green-950' : 'text-gray-400 bg-gray-800'
//                 }`}>
//                   {isActive ? 'Active' : new Date(contest.startTime) > now ? 'Upcoming' : 'Ended'}
//                 </span>
//               </div>
//               <p className="text-gray-400 text-sm mb-4 leading-relaxed">{contest.description}</p>
...
//               className={`px-5 py-2.5 text-sm capitalize border-b-2 transition-colors font-medium ${
...
//               <p className="text-gray-500 text-sm text-center py-12">No problems in this contest.</p>
//             ) : contest.problems.map((problem, i) => (
//               <div key={problem._id} onClick={() => navigate(`/problem/${problem._id}?contestId=${id}`)}
//                 className="grid grid-cols-12 gap-4 items-center px-5 py-4 rounded-xl bg-[#1a1d27] hover:bg-[#1e2130] border border-transparent hover:border-gray-700 cursor-pointer transition-all"
//               >
//                 <div className="col-span-1 text-gray-500 text-sm font-medium">{i + 1}</div>
//                 <div className="col-span-7 text-sm font-medium text-white">{problem.title}</div>
//                 <div className="col-span-2">
//                   <span className={`text-xs px-2.5 py-1 rounded-lg font-medium ${diffStyle[problem.difficulty]}`}>
//                     {problem.difficulty}
//                   </span>
//                 </div>
//                 <div className="col-span-2 text-xs text-gray-500">{problem.tags?.[0] || ''}</div>
//               </div>
//             ))}
//           </div>
//         )}
...
//             <p className="text-gray-500 text-sm text-center py-12">No submissions yet.</p>
//           ) : (
...
//                   <div key={entry.user._id} className={`grid grid-cols-12 gap-4 items-center px-6 py-4 ${
//                     i === 0 ? 'bg-yellow-950 bg-opacity-30' :
//                     i === 1 ? 'bg-gray-700 bg-opacity-10' :
//                     i === 2 ? 'bg-orange-950 bg-opacity-20' : 'hover:bg-[#1e2130]'
//                   } transition-colors`}>
...
//                     </div>
//                     <div className="col-span-2 text-sm font-bold text-green-400">{entry.solvedCount}</div>
//                     <div className="col-span-2 text-sm text-gray-400">{entry.total}</div>
//                   </div>
//                 ))}
//               </div>
//             </div>
...
const ContestTimer = ({ contest }) => {
  ...
  const isActive   = new Date(contest.startTime) <= now && new Date(contest.endTime) > now
  const target     = isUpcoming ? contest.startTime : isActive ? contest.endTime : null
  const timeLeft   = useCountdown(target)
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-2.5 rounded-xl border flex-shrink-0 ${
      urgent     ? 'bg-red-950 border-red-800'   :
      isActive   ? 'bg-green-950 border-green-800' :
                   'bg-blue-950 border-blue-800'
    }`}>
...
const ContestDetails = () => {
  ...
  // Check if current user has joined — participants can be objects or plain IDs
  const hasJoined = contest?.participants?.some(p => {
    const pid = p._id || p
    return pid?.toString() === user?._id?.toString()
  }) ?? false

  // Admin always sees problems. Regular users: must be joined AND (active OR ended)
  ...
  const leaderboard = Object.values(
    submissions.reduce((acc, s) => {
      const uid = s.user?._id
      if (!uid) return acc
      if (!acc[uid]) acc[uid] = { user: s.user, solved: new Set(), attempts: 0 }
      acc[uid].attempts++
      if (s.status === 'Accepted') acc[uid].solved.add(s.problem?._id)
      return acc
    }, {})
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mb-2 flex-wrap">
                <h1 className="text-xl font-semibold">{contest.title}</h1>
                <span className={`text-xs px-2.5 py-1 rounded-lg font-medium ${
                  isActive   ? 'text-green-400 bg-green-950' :
                  isUpcoming ? 'text-blue-400  bg-blue-950'  :
            ...
            <div className="flex flex-col items-end gap-3 flex-shrink-0">
              ...
              {/* Join button — hide for admin and already joined and ended contests */}
              {!isAdmin && !hasJoined && !isEnded && (
                <button
                  onClick={handleJoin}
                  disabled={joining}
                  className="px-5 py-2 bg-green-600 hover:bg-green-500 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-xl transition-colors"
                >
                  {joining ? 'Joining...' : 'Join Contest'}
                </button>
              )}

              {/* Joined but waiting for start */}
        ...
        {tab === 'problems' && (
          <div>
            ...
            {/* NOT joined and not admin */}
            {!canSeeProblems && !hasJoined && (
              <div className="text-center py-16 bg-[#1a1d27] border border-gray-800 rounded-2xl">
                <div className="text-4xl mb-4">🔒</div>
                <p className="text-white font-semibold mb-2">Register to access problems</p>
            ...
            {/* Joined but contest hasn't started yet (non-admin) */}
            ...
            {canSeeProblems && (
              <div className="space-y-2">
                {!contest.problems?.length ? (
                  ...
                  contest.problems.map((problem, i) => {
                    const solvedInContest = submissions.some(
                    ...
                    return (
                      <div
                        key={problem._id}
                        onClick={() => navigate(`/problem/${problem._id}?contestId=${id}`)}
                        className={`grid grid-cols-12 gap-4 items-center px-5 py-4 rounded-xl border cursor-pointer transition-all ${
                          solvedInContest
                      ...
                      >
                        ...
                        <div className={`col-span-7 text-sm font-medium ${solvedInContest ? 'text-green-300' : 'text-white'}`}>
                          {problem.title}
                        </div>
                        <div className="col-span-2">
                          <span className={`text-xs px-2.5 py-1 rounded-lg font-medium ${diffStyle[problem.difficulty]}`}>
        ...
        {tab === 'leaderboard' && (
          leaderboard.length === 0 ? (
            <div className="text-center py-16 bg-[#1a1d27] border border-gray-800 rounded-2xl">
              <div className="text-4xl mb-4">🏆</div>
              <p className="text-gray-400 text-sm">No submissions yet. Be the first!</p>
            </div>
          ) : (
            <div className="bg-[#1a1d27] border border-gray-800 rounded-2xl overflow-hidden">
              <div className="grid grid-cols-12 gap-4 text-xs font-medium text-gray-500 uppercase tracking-wider px-6 py-4 border-b border-gray-800">
              ...
              <div className="divide-y divide-gray-800">
                {leaderboard.map((entry, i) => {
                  ...
                  return (
                    <div key={entry.user._id} className={`grid grid-cols-12 gap-4 items-center px-6 py-4 transition-colors ${
                      isMe    ? 'bg-green-950 bg-opacity-20' :
                      i === 0 ? 'bg-yellow-950 bg-opacity-20' :
                      i === 1 ? 'bg-gray-600 bg-opacity-10' :
                      i === 2 ? 'bg-orange-950 bg-opacity-10' :
                                'hover:bg-[#1e2130]'
                    }`}>
                      <div className="col-span-1 text-sm font-bold">
                        {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `#${i + 1}`}
                      ...
                      <div className="col-span-6 flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium text-white ${isMe ? 'bg-green-700' : 'bg-gray-700'}`}>
                          {entry.user.username?.[0]?.toUpperCase()}
                        </div>
